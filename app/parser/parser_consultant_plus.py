...
class ConsultantPlusParser():
    ...
    def _get_data(self):
        ...
        for month_table in month_tables:
            month = month_table.find('th', class_="month")
            ...
            for day_table in day_tables:
                ...
                day = day_table.text
                if len(class_attr) == 0:
                    result.append((self.YEAR, month.text, day))
                elif 'weekend' in class_attr:
                    result.append((self.YEAR, month.text, day, 'weekend'))
                elif 'preholiday' in class_attr:
                    result.append((self.YEAR, month.text, day, 'preholiday'))
